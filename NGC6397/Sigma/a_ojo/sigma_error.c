...
int main (void)
{
  ...
  for(j=0.0; j<10.0; j=j+1.0)
    ...
    //fprintf( script, "replot 'sigma.dat' u 1:2:5 with labels offset char 1,1\n");
    fprintf( script, "set grid\nset terminal png\nset output 'Sigma2.png'\nset nokey\n");
    fprintf( script, "set title 'Sigma vs Radius'\n" );
    //fprintf( script, "set logscale x\n" );
    //fprintf( script, "set boxwidth 0.3\n");
    //fprintf(script, "set offset 1,1,1,1\n");
    fprintf( script, "set xlabel 'Radius in Arcmin'\n" );
    fprintf( script, "set xrange [0:16]\n" );
    fprintf( script, "set ylabel 'Sigma (km/s)'\n" );
    //fprintf( script, "replot 'sigma.dat' u 1:2:6 with labels offset char 4.7,1\n");
    fprintf( script, "replot 'sigma.dat' u 1:2:(sprintf("[%.2f,%.2f]",$5,$6)) with labels offset char 1,1\n" );
