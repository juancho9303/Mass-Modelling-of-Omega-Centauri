...
double int_2 (double r, void * params) 
{
  ...
  double int_2 = (1.0 - beta*((R*R)/(r*r)))*((r)/(sqrt(r*r-R*R)))*(((12.0*pow((a_dm+r),4.0)*log((a_dm+r)/(r))) - a_dm*(25.0*a_dm*a_dm*a_dm+52.0*a_dm*a_dm*r+42.0*a_dm*r*r+12.0*r*r*r))/(12.0*pow(a_dm,5)*pow((a_dm+r),4)));
  return int_2;
}
...
int main(){
  ...
  for(R = 0.01; R < N; R=R+0.01){
	...
	I_R = (M/(2.0*M_PI*a_dm*a_dm*GAMMA*(1.0-s*s)*(1.0-s*s)))*((2.0+s*s)*X_s-3.0);
	sig_p = ((2.0*G*M*M*a_dm)/(GAMMA*I_R*2.0*M_PI))*re2;
