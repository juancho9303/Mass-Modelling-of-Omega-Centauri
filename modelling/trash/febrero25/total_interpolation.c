...
int main()
{
  ...
  for(a_dm = 0.005; a_dm <= 0.06;  a_dm = a_dm + 0.001)
    {
      for(a_s = 0.005; a_s <= 0.06;  a_s = a_s + 0.001)
	{
	  // PARA EVITAR INDETERMINACIONES EN ALGUNOS DENOMINADORES
	  if (a_s != a_dm)
	    {
	      for(M_dm = 0.00001; M_dm < 0.0008;  M_dm = M_dm + 0.00001)
		{
		  for(M_s = 0.00001; M_s < 0.0008;  M_s = M_s + 0.00001)
		    {
		      params.beta  =  beta;
		      params.a_s   =  a_s;
		      params.a_dm  =  a_dm;
