...
double integrando(double r, void * params) 
{
  ...
  double alpha, a1, a2, a3, a4, a5, b1, b2, b3, b4, b5, M_sM_dm, M_dmM_s, M_sM_s, M_dmM_dm, shi_t_min;
...
int main()
{
  ...
  double X_s, I_R, s, R, R_min, sig_p, rho, re1, re2, result1, error1, result2, error2, a_dm, a_s, M_s, M_dm, beta, R_o[12], sig_p_o[12], shi2, shi_t, otra, a_dm_min, a_s_min, M_dm_min, M_s_min;
  double shi_min = 1000000.0;
  //double * small = malloc (1 * sizeof(double));
	  ...
	  if (a_s != a_dm)
	      ...
	      for(M_dm = 0.00001; M_dm < 0.0008;  M_dm = M_dm + 0.0002)
		{
		  for(M_s = 0.0001; M_s < 0.0008;  M_s = M_s + 0.0002)
			...
			//fprintf (shi_cua, "%.18f\n", shi_t);
			//fprintf(mod,"%16.8e\t %16.8e\t %16.8e\t %16.8e\n", a_dm_min, a_s_min, M_dm_min, M_s_min);
			//printf ("%.18f\n", shi_t);
			shi_t = 0.0;
