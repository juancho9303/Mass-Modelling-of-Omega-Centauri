...
int main(int argc, char *argv[])
{
  ...
  gamma_ini  = 0.1;        gamma_end  = 3.0;      Delta_gamma  = 1.0;
  adm_ini    = 1.0;        adm_end    = 60.0;     Delta_adm    = 20.0;
  as_ini     = 1.0;        as_end     = 60.0;      Delta_as    = 20.0;
  Mdm_ini    = 1.0;        Mdm_end    = 80.0;     Delta_Mdm    = 20.0;
  Ms_ini     = 1.0;        Ms_end     = 80.0;     Delta_Ms     = 20.0;
  beta_ini   = 0.0001;     beta_end   = 1.0;      Delta_beta   = 0.4;

  // THIS LINE CALCULATES THE NUMBER OF ITERATIONS FOR THE USER'S REFERENCE
  double Ntotal_iterations;
  int counter, cuenta;
  Ntotal_iterations = (double) ( ((beta_end-beta_ini)/Delta_beta)*((gamma_end-gamma_ini)/Delta_gamma)*((adm_end-adm_ini)/Delta_adm)*((as_end-as_ini)/Delta_as)*((Mdm_end-Mdm_ini)/Delta_Mdm)*((Ms_end-Ms_ini)/Delta_Ms) );
  
  printf("  Running %lf iterations in parameter space\n", Ntotal_iterations);
		      ...
		      for(M_dm = Mdm_ini; M_dm < Mdm_end;  M_dm = M_dm + Delta_Mdm)
			{
			  for(M_s = Ms_ini; M_s < Ms_end;  M_s = M_s + Delta_Ms)
			      ...
			      if(chi2 < chi_min)
				{
			      ...
			      if((counter%1000) == 0)
				printf("Ready %d iterations of %lf\n",counter, Ntotal_iterations);
			      counter++;
