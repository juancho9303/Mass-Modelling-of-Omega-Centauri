...
double integrando(double r, void * params) 
{
  ...
  M_sM_s = M_s*M_s*a_s* ( (12.0*pow(aux1,4.0)*log(aux1/r)) - a_s*(25.0*a_s*a_s*a_s + 52.0*a_s*a_s*r + 42.0*a_s*r*r + 12.0*r*r*r) ) / (12.0*pow(a_s,5)*pow(aux1,4));
  
  /* This is D in our notation */
  M_dmM_dm = M_dm*M_dm*a_dm * ( (12.0*pow(aux2,4.0)*log(aux2/r)) - a_dm*(25.0*a_dm*a_dm*a_dm + 52.0*a_dm*a_dm*r + 42.0*a_dm*r*r + 12.0*r*r*r) ) / (12.0*pow(a_dm,5)*pow(aux2,4));
  
  /*  
      b1 = 2.0*pow(a_s,3.0) * pow((a_s-a_dm),4.0) * a_dm*a_dm*aux1*aux1*aux2;
      ...
      c1 = 2.0*pow(a_dm,3) * pow((a_dm-a_s),4.0) * a_s*a_s*aux2*aux2*aux1;
