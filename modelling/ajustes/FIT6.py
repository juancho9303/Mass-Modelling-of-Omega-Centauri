...
dis, bri = np.loadtxt("data_ready_ln.dat", usecols=(0,1), unpack= True)  
...
def I_R(R,a,GAMMA,M):
  ...
  y = np.piecewise(R, 
		   [R < a, R == a, R > a], 
		   [lambda R: np.log(M/(2.0*np.pi*GAMMA*a*a*(1.0 - (R/a)**2)**2)*((2.0 + (R/a)**2)*((1.0/(np.sqrt(1.0 -(R/a)**2)))*(np.log((1.0 + np.sqrt(1.0 - (R/a)**2))/(R/a)))) - 3.0)),
                   lambda R: np.log(2.0*M/(15*np.pi*a**2*GAMMA)),
                   lambda R: np.log(M/(2.0*np.pi*GAMMA*a*a*(1.0 - (R/a)**2)**2)*((2.0 + (R/a)**2)*((1.0/(np.sqrt((R/a)**2 - 1.0)))*(np.arccos(a/R))) - 3.0))])
  return y
