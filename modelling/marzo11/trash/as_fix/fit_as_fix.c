...
int evaluate_integral(double R, double beta, double gamma, double a_dm, double a_s, double M_dm, double M_s, double *sig_p, double *R_arcmin)
{
  ...
  */
  ...
  gsl_integration_qag(&F1, R, LIMIT_RADIUS, 1.0e-5, 1e-4, Nint, 1, z, &result, &error); 
  //printf("R = %lf I=%lf\n",R, result);
...
int main(int argc, char *argv[])
{
  ...
  gamma_ini  = 0.1;        gamma_end  = 3.0;      Delta_gamma  = 0.7;
  adm_ini    = 1.0;        adm_end    = 60.0;     Delta_adm    = 8.0;
  Mdm_ini    = 1.0;        Mdm_end    = 80.0;     Delta_Mdm    = 8.0;
  Ms_ini     = 1.0;        Ms_end     = 80.0;     Delta_Ms     = 8.0;
  beta_ini   = 0.0001;     beta_end   = 1.0;      Delta_beta   = 0.3;
  ...
  fprintf( script, "set yrange [3:14]\n" );
