...
def I_R(R,a,GAMMA,M):
  ...
  y = np.piecewise(R, 
		   [R < a, R == a, R > a], 
		   [lambda R: M/(2.0*np.pi*GAMMA*a*a*(1.0 - (R/a)**2)**2)*((2.0 + (R/a)**2)*((1.0/(np.sqrt(1.0 -(R/a)**2)))*(1.0/np.arccosh(R/a))) - 3.0),
                   lambda R: 2.0*M/(15*np.pi*a**2*GAMMA),
                   lambda R: M/(2.0*np.pi*GAMMA*a*a*(1.0 - (R/a)**2)**2)*((2.0 + (R/a)**2)*((1.0/(np.sqrt((R/a)**2 - 1.0)))*(1.0/np.arccos(R/a))) - 3.0)])
  return y
