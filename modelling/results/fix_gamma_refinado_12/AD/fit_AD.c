...
int main(int argc, char *argv[])
{
  ...
  adm_ini    = 15.0;        adm_end    = 17.0;     Delta_adm    = 0.2;
  as_ini     = 54.0;        as_end     = 56.0;     Delta_as     = 0.2;
  Mdm_ini    = 15.0;        Mdm_end    = 17.0;     Delta_Mdm    = 0.2;
  Ms_ini     = 21.0;        Ms_end     = 23.0;     Delta_Ms     = 0.2;
  beta_ini   = 0.1;         beta_end   = 0.5;      Delta_beta   = 0.05;

  // THIS LINE CALCULATES THE NUMBER OF ITERATIONS FOR THE USER'S REFERENCE
  int Ntotal_iterations, counter, cuenta;
  Ntotal_iterations = (int) ( ((beta_end-beta_ini)/Delta_beta)*((adm_end-adm_ini)/Delta_adm)*((Mdm_end-Mdm_ini)/Delta_Mdm)*((as_end-as_ini)/Delta_as)*((Ms_end-Ms_ini)/Delta_Ms) );
