...
int evaluate_integral(double R, double beta, double gamma, double a_dm, double a_s, double M_dm, double M_s, double *sig_p, double *R_arcmin)
{
  ...
  */
  ...
  gsl_integration_qag(&F1, R, LIMIT_RADIUS, 1.0e-7, 1e-6, Nint, 1, z, &result, &error); 
  //printf("R = %lf I=%lf\n",R, result);
...
int main(int argc, char *argv[])
{
  ...
  adm_ini    = 1.0;        adm_end    = 5.0;      Delta_adm    = 0.3;
  Mdm_ini    = 1.0;        Mdm_end    = 5.0;      Delta_Mdm    = 0.3;
  Ms_ini     = 3.0;        Ms_end     = 7.0;      Delta_Ms     = 0.3;
  beta_ini   = 0.001;      beta_end   = 0.5;      Delta_beta   = 0.03;

  // THIS LINE CALCULATES THE NUMBER OF ITERATIONS FOR THE USER'S REFERENCE
  int Ntotal_iterations, counter, cuenta;
  Ntotal_iterations = (int) ( ((beta_end-beta_ini)/Delta_beta)*((adm_end-adm_ini)/Delta_adm)*((Mdm_end-Mdm_ini)/Delta_Mdm)*((Ms_end-Ms_ini)/Delta_Ms) );
